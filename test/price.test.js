...
test("returns open graph value (og:price:amount)", async () => {
  ...
  expect(metadata.price).toBe(100000000.0);
});
...
test("overstock.com ld+json", async () => {
  ...
  const url =
    "https://www.overstock.com/Home-Garden/Certified-International-Elegance-Canape-Plates-Set-of-6/29099468/product.html?refccid=LDNVKF2XQ7MKQP2LE3VXMUYALU&searchidx=0&guid=f4adad64-034d-46b2-92dc-e956d094d3ea&osp=true&option=51668490&kwds=&rfmt=";
...
test("jossandmain.com ld+json", async () => {
  ...
  const url =
    "https://www.jossandmain.com/lighting/pdp/duprey-4-light-lantern-pendant-j001109161.html";
  const metadata = await metascraper({ url, html });
...
test("costco.com product:price:amount", async () => {
  ...
  const url =
    "https://www.costco.com/afa-stainless-33-inch-sink-and-semi-pro-faucet-combo.product.100432602.html";
  const metadata = await metascraper({ url, html });
...
test("rejuvenation.com item on sale", async () => {
  ...
  const url =
    "https://www.rejuvenation.com/catalog/collections/keystick-wall-sconce/products/5c50a4d2ccf2f73ea295b9e1";
  const metadata = await metascraper({ url, html });
  expect(metadata.price).toBe(120.0);
});
...
test("amazon.com #attach-base-product-price price", async () => {
  const html = await readFile(
    resolve(__dirname, "fixtures/amazon-gorilla.html")
  );
  const url =
    "https://www.amazon.com/Gorilla-6012110-Black-2-Pack-Piece/dp/B00N150YS4/ref=sr_1_3?dchild=1&keywords=gorilla+tape&qid=1630396817&sr=8-3";
  const metadata = await metascraper({ url, html });
  expect(metadata.price).toBe(12.48);
});

test("globalindustrial.com NaN", async () => {
  const html = await readFile(
    resolve(__dirname, "fixtures/globalindustrial.html")
  );
  const url =
    "https://www.globalindustrial.com/p/work-benches/components/tops/60-x-24-maple-square-edge-bench-top";
  const metadata = await metascraper({ url, html });
  expect(metadata.price).toBeFalsy();
});
